...
**Phase 4 — Local translation** is implemented.
...
- In-memory LRU translation cache; captured images and recognized text are not written to disk.
- OCR text normalization that retains Unicode, line breaks, and likely chat username prefixes.
- Background capture, OCR, detection, model installation, and translation tasks so the UI remains responsive.
- Timing, confidence, route, and cache status information.
...
6. Use **Force read**, **Pause**, **Resume**, **Swap**, **Copy**, and **Lock lens** as needed.
...
BeeTales has no telemetry. Screenshots, OCR text, and translations remain in memory. Network access is only needed to download OCR or translation model files after confirmation; inference is local afterward. Translation models are installed per language direction, so `en -> es` and `es -> en` are separate packages.
...
The suite covers settings, geometry, multi-monitor capture, change detection, preprocessing, OCR parsing and caching, language detection, text normalization, direct and pivot route selection, translation caching, Argos translation behavior, and model installation decisions.
...
4. Force-read the same text and confirm that the session cache is reported.
...
10. Disconnect from the network after installing the models and confirm capture, OCR, detection, and translation still work.
...
5. **Phase 5 — Continuous processing:** performance controls and optional history.
